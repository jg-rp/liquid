...
def test_base64_url_safe_decode_filter(case: Case) -> None:
    if isclass(case.expect) and issubclass(case.expect, Error):
        with pytest.raises(case.expect):
            base64_url_safe_decode(case.val, *case.args, **case.kwargs)
    else:
        assert (
            base64_url_safe_decode(case.val, *case.args, **case.kwargs) == case.expect
        )

...
def test_base64_url_safe_encode_filter(case: Case) -> None:
    if isclass(case.expect) and issubclass(case.expect, LiquidError):
        with pytest.raises(case.expect):
            base64_url_safe_encode(case.val, *case.args, **case.kwargs)
    else:
        assert (
            base64_url_safe_encode(case.val, *case.args, **case.kwargs) == case.expect
        )

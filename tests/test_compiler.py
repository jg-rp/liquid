    ...
    def test_boolean_expressions(self):
        ...
        test_cases = [
            Case(
                ...
                source=r"{{ true }}",
                expected_constants=[],
                expected_instructions=[code.make(Opcode.TRU), code.make(Opcode.POP)],
            ),
            Case(
                description="literal false",
                source=r"{{ false }}",
                expected_constants=[],
                expected_instructions=[code.make(Opcode.FAL), code.make(Opcode.POP)],
            ),
            Case(
    ...
    def test_conditionals(self):
        ...
        test_cases = [
            Case(
                ...
                expected_instructions=[
                    code.make(Opcode.TRU),
                    code.make(Opcode.STJ),
                    code.make(Opcode.JIN, 5),
                    code.make(Opcode.CONSTANT, 0),
                    code.make(Opcode.JMP, 6),
        ...
        test_cases = [
            Case(
                ...
                expected_instructions=[
                    ...
                    code.make(Opcode.EQ),
                    code.make(Opcode.STJ),
                    code.make(Opcode.JIN, 7),
                    code.make(Opcode.CONSTANT, 0),
                    code.make(Opcode.JMP, 15),
                    code.make(Opcode.TRU),
                    code.make(Opcode.TRU),
                    code.make(Opcode.EQ),
                    code.make(Opcode.STJ),
                    code.make(Opcode.JIN, 14),
                    code.make(Opcode.CONSTANT, 1),
                    code.make(Opcode.JMP, 15),
                    code.make(Opcode.NOP),
                    code.make(Opcode.POP),
        ...
        test_cases = [
            Case(
                ...
                expected_instructions=[
                    ...
                    code.make(Opcode.FOR, 5, 0),  # number of block and free vars
                ],
            ),
            Case(
                ...
                expected_constants=[
                    ...
                    (
                        ...
                        code.make(Opcode.CONSTANT, 1),  # range stop
                        code.make(Opcode.CONSTANT, 2),  # range start
                        code.make(Opcode.FOR, 5, 1),  # index of loop var
                        code.make(Opcode.STE, 0),
                        code.make(Opcode.JMP, 1),
                        code.make(Opcode.NOP),
                ...
                expected_instructions=[
                    ...
                    code.make(Opcode.FOR, 5, 0),  # index of loop var
                ],
            ),
        ]

...
class CaseNode(ast.Node):
    __slots__ = ("tok", "condition", "whens", "default")
...
class CaseTag(Tag):
    ...
    def parse(self, stream: TokenStream) -> ast.Node:
        ...
        tag = CaseNode(tok, condition=parse_filtered_expression(expr_iter),)
        stream.next_token()
        ...
        while stream.current.istag(TAG_WHEN):
            ...
            expr_iter = lexer.tokenize(f"{tag.condition} == {stream.current.value}")
            expr = parse_boolean_expression(expr_iter)

            when = ast.ConditionalBlockNode(stream.current, condition=expr,)

            stream.next_token()
            when.block = parser.parse_block(stream, ENDWHENBLOCK)
            tag.whens.append(when)

        if stream.current.istag(TAG_ELSE):
            stream.next_token()
            tag.default = parser.parse_block(stream, ENDCASEBLOCK)
